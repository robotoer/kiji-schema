 ...
 *
 * @deprecated Use {@link org.kiji.common.tools.KijiToolLauncher} instead.
 */
@ApiAudience.Public
...
public final class KijiToolLauncher extends Configured {
  /**
   * Instance of the new tool launcher for running new tools.
   */
  private final org.kiji.common.tools.KijiToolLauncher mNewLauncher;

  /**
   * Constructs a tool launcher.
   */
  public KijiToolLauncher() {
    mNewLauncher = new org.kiji.common.tools.KijiToolLauncher();
  }
  ...
  public int run(String[] args) throws Exception {
    ...
    final org.kiji.common.tools.KijiTool newTool = mNewLauncher.getToolForName(toolName);
    final KijiTool oldTool = getToolForName(toolName);

    if (null == oldTool && null == newTool) {
      ...
      return 1;
    } else if (null == oldTool) {
      // Continue on to Hadoop property argument parsing and tool launch.
      return mNewLauncher.run(newTool, nonToolNameArgs);
    } else if (null == newTool) {
      // Continue on to Hadoop property argument parsing and tool launch.
      return run(oldTool, nonToolNameArgs);
    } else {
